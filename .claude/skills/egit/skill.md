...
**選択肢2（GitHub連携）の場合:**
  a. `gh auth status` で認証チェック。未認証なら「GitHubを使うには認証が必要です。`! gh auth login` と入力してもらえますか？」
  b. `git init`
  c. デフォルトブランチが `master` の場合は `git branch -m master main` で `main` にリネーム
  d. `.gitignore` 作成
  e. プロジェクト名からリポジトリ名を提案し確認
  f. 公開/非公開を確認:
  ```
  ...
  2. 公開（誰でも見える）
  ```
  g. `gh repo create` でリポジトリ作成
  h. 全ファイルを初回セーブ＆アップロード
  i. 報告: 「環境をつくりました！ GitHubにもバックアップされます」
...
4. 変更があれば:
   a. **メインの保管場所にいる場合** → 作業場所を自動作成し、理由を説明:
      ```
      メインの保管場所を直接変えないように、作業場所を作りました。
      （作業場所: feature/xxx）
      作業場所では自由に変更できて、完成したらメインに合流できます。
      ```
      - 変更内容から名前を推測: `feature/[英語の機能名]`
   b. 変更内容を分析し、わかりやすい日本語でセーブメッセージを自動生成
      - 形式: `種別: 説明` （例: `追加: ログイン画面のUIを実装`）
...
**トリガー**: `/egit 合流`、または「メインに合流して」「作業おわった」「本番に反映して」等の発言
...
5. **競合が発生した場合** → わかりやすく説明。表現は操作の方向で使い分ける:
   ...
   **取り込み時（メインの変更 → 自分の作業場所に持ってくる）:**
   ```
   同じ箇所が別々に変更されていました！

   ファイル: src/app.js（23行目あたり）
     取込（もってくる側）の変更: color = "green"
     あなたの変更:               color = "red"
...
3. 既にセーブ済みのファイルの場合 → 追跡を解除（`git rm --cached`）し、理由を説明:
   ...
   ```
4. まだセーブされていないファイルの場合 → `.gitignore` への追加だけで完了:
   ```
   ○○を無視リストに追加しました。
...
|---|---------|------|---------|--------------|
| 1 | feature/add-login | 作業中 | 2026-04-03 | ログイン画面のUIとバリデーションを実装中 |
| 2 | feature/fix-header | 合流済 | 2026-04-02 | ヘッダーのレイアウト崩れを修正 |
| 3 | feature/add-contact | 一時保管 | 2026-04-01 | お問い合わせフォームを途中まで作成 |
```
...
2. 「作業中」または「一時保管」の作業場所を提案:
   ...
   1. 「ログイン画面のUI実装」の続き（feature/add-login）
   2. 「お問い合わせフォーム」の続き（feature/add-contact・一時保管あり）
   3. やっぱりやめる
   ```
3. ユーザーが選択したら:
...
UserPromptSubmitフックから未セーブの変更情報が注入されることがある。
その場合:

1. **変更5個以上**:
   回答の最後に: 「ちなみに、変更が溜まっています。「セーブして」と言ってもらえればセーブしますよ」
2. **変更10個以上**:
   より強く: 「変更がかなり溜まっています！ 「セーブして」と言ってもらえればすぐセーブします」
